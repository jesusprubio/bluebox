...
module.exports.createLoginPairsOLD = function (users, passwords, userAsPass) {
    var pairs = [];
...
module.exports.createLoginPairs = function (users, passwords, userAsPass) {
    ...
    for (var i = 0; i < users.length; i++) {
        if(userAsPass && (users[i] !== passwords[0])) {
            pairs.push({
                user : users[i],
                pass : users[i]
            });
        ...
        for (var j = 0; j < passwords.length; j++) {
            console.log('USERR: ' + users[i]);
            console.log('PASSS: ' + passwords[j]);
            pairs.push({
                user : users[i],
                pass : passwords[j]
            });
