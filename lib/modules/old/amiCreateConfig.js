...
module.exports.help = {
  description: 'Use the Asterisk Manager service (AMI) to change a config file',
  options: {
    target: {
      type: 'ip',
    ...
    filename: {
      type: 'allValid',
      description: 'Name of the config file to change',
      defaultValue: 'manager.conf',
    },
    timeout: {
      type: 'positiveInt',
...
module.exports.run = (options, callback) => {
  ...
  ami.on('namiConnected', () => {
    const action = new namiLib.Actions.CreateConfig();

    action.filename = options.filename;
    connected = true;
    ami.send(action, (res) => {
      ami.close();
      callback(null, res);
