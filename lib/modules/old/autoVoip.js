...
module.exports.run = (options, callback) => {
  ...
  async.series([
    async0Cb => {
      ...
      // async0Cb();
      // } else {
      initialTargets = networkUtils.createAutoTargets(
        options.targets,
        profile.sipServices,
    ...
    async0Cb => {
      ...
      utils.each(profile.slowTypes, meth => {
        utils.each(Object.keys(report), ipAddress => {
          finalPairs.push({
            ipAddress,
            meth,
          });
    ...
    async0Cb => {
      ...
      utils.each(profile.bruteExtTypes, meth => {
        utils.each(Object.keys(report), ipAddress => {
          finalTargets.push({
            ipAddress,
            meth,
      ...
      async.eachSeries(Object.keys(report), (ipAddress, async1Cb) => {
        ...
        utils.each(profile.bruteExtTypes, type => {
          if (report[ipAddress].sipBruteExt100 &&
            ...
            utils.each(report[ipAddress].sipBruteExt100[type].valid, extObj => {
              finalExtensions.push(extObj.extension);
            });
          }
        });
        // removing duplicates
        finalExtensions = utils.unique(finalExtensions);
