...
module.exports.userAgent = pkt => {
  const userLine = utils.filter(splitMsg(pkt), line => grammar.userRE.test(line));
...
module.exports.code = pkt => {
  const codeLine = utils.filter(splitMsg(pkt), line => grammar.codeLineRE.test(line));
...
module.exports.server = pkt => {
  const serverLine = utils.filter(splitMsg(pkt), line => { grammar.serverRE.test(line); });
...
module.exports.organization = pkt => {
  const orgLine = utils.filter(splitMsg(pkt), line => grammar.orgRE.test(line));
...
module.exports.realmNonce = pkt => {
  ...
  let isProxy = false;
  let authLine = utils.filter(splittedMsg, parseAuth);
  let authSplit;
  let nonce;
  let realm;
